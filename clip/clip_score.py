...
def main():
    ...
    image_ids = [int(x[:-2]) for x in image_ids]
    image_ids = np.sort(image_ids)
    image_ids = [str(x) for x in image_ids]
    #print(image_ids)
    ###Not working because you need to sort image ID
    with open(args.candidates_json) as f:
        candidates = json.load(f)
    print(candidates[0].get(image_ids[0]))
    print(image_ids[0])
    
    #print(image_ids)
    #print(candidates)
    candidates = [candidates[index].get(cid) for index, cid in enumerate(image_ids)]
